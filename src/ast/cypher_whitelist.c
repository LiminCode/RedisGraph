...
static AST_Validation _CypherWhitelist_ValidateQuery(const cypher_astnode_t *elem) {
	...
	const cypher_operator_t *operator1 = NULL;
	if(type == CYPHER_AST_BINARY_OPERATOR) {
		operator1 = cypher_ast_binary_operator_get_operator(elem);
	} else if(type == CYPHER_AST_UNARY_OPERATOR) {
		operator1 = cypher_ast_unary_operator_get_operator(elem);
	}
	if(operator1) {
		if(raxFind(_operator_whitelist, (unsigned char *)operator1, sizeof(*operator1)) == raxNotFound) {
			QueryCtx_SetError("RedisGraph does not currently support %s", operator1->str);
			return AST_INVALID;
		}
	}
