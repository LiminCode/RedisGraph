...
static AR_ExpNode *_AR_EXP_FromUnaryOpExpression(const cypher_astnode_t *expr) {
	...
	const cypher_operator_t *operator1 = cypher_ast_unary_operator_get_operator(expr);

	if(operator1 == CYPHER_OP_UNARY_MINUS) {
...
static AR_ExpNode *_AR_EXP_FromBinaryOpExpression(const cypher_astnode_t *expr) {
	const cypher_operator_t *operator1 = cypher_ast_binary_operator_get_operator(expr);
	AST_Operator operator_enum = AST_ConvertOperatorNode(operator1);
...
static AR_ExpNode *_AR_EXP_FromComparisonExpression(const cypher_astnode_t *expr) {
	...
	if(length > 1) {
		...
		for(uint i = 0; i < length; i++) {
			const cypher_operator_t *operator1 = cypher_ast_comparison_get_operator(expr, i);
			AST_Operator operator_enum = AST_ConvertOperatorNode(operator1);
		...
		const cypher_operator_t *operator1 = cypher_ast_comparison_get_operator(expr, 0);
		AST_Operator operator_enum = AST_ConvertOperatorNode(operator1);
