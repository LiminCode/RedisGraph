...
static ExecutionPlan *_ClonePlanInternals(const ExecutionPlan *template1) {
	ExecutionPlan *clone = ExecutionPlan_NewEmptyExecutionPlan();

	clone->record_map = raxClone(template1->record_map);
	if(template1->ast_segment) clone->ast_segment = AST_ShallowCopy(template1->ast_segment);
	if(template1->query_graph) {
		QueryGraph_ResolveUnknownRelIDs(template1->query_graph);
		clone->query_graph = QueryGraph_Clone(template1->query_graph);
	}
	// TODO improve QueryGraph logic so that we do not need to store or clone connected_components.
	if(template1->connected_components) {
		array_clone_with_cb(clone->connected_components, template1->connected_components, QueryGraph_Clone);
	}
...
static ExecutionPlan *_ExecutionPlan_Clone(const ExecutionPlan *template1) {
	OpBase *clone_root = _CloneOpTree(NULL, template1->root, NULL);
...
ExecutionPlan *ExecutionPlan_Clone(const ExecutionPlan *template1) {
	ASSERT(template1 != NULL);
	// Store the original AST pointer.
	AST *master_ast = QueryCtx_GetAST();
	// Verify that the execution plan template1 is not prepared yet.
	ASSERT(template1->prepared == false && "Execution plan cloning should be only on templates");
	ExecutionPlan *clone = _ExecutionPlan_Clone(template1);
